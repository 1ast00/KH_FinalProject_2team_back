...
public class AdminReviewsService {
    ...
    public AdminReviewsPageDTO getPage(String q, Integer status, int p, int size) {
        ...
        List<AdminReviewsListItemDTO> items = mapper.selectReviewsPage(q, status, offset, pageSize);
    ...
    public AdminReviewDetailDTO getDetail(Long brno) {
        ...
        d.setComments(mapper.selectRecentReviewComments(brno, 5));
        return d;
    }

    public boolean toggleStatus(long brno, boolean posted) {
        return mapper.updateReviewStatus(brno, posted ? 1 : 0) > 0;
    }
}
